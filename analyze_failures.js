...
function loadDebug(plannerDebugPath) {
  const basename = path.basename(plannerDebugPath);
  try {
    return JSON.parse(fs.readFileSync(LOGS_DIR + basename, "utf8"));
  } catch {
    return null;
  }
}
...
const allRequests = files.map(f => {
  const data = JSON.parse(fs.readFileSync(REQUESTS_DIR + f, "utf8"));
  data._filename = f;
  return data;
});
...
for (let i = 0; i < invalidResponseSamples.length; i++) {
  ...
  // Show each model response and the error that followed
...
for (let i = 0; i < jsonFilterSamples.length; i++) {
  ...
  // Find json_filter tool calls and their errors
  for (const event of (debug.events || [])) {
...
for (let i = 0; i < toolCallLimitSamples.length; i++) {
  ...
  // Count tool calls and show the pattern
...
// Collect all invalid response texts
const invalidTexts = [];
...
for (const text of invalidTexts) {
  ...
  if (!trimmed) { patterns.empty_or_whitespace++; continue; }
  if (trimmed.startsWith("{")) patterns.starts_with_json_brace++;
  if (trimmed.startsWith("[")) patterns.starts_with_json_bracket++;
  if (trimmed.includes("<think>") || trimmed.includes("</think>")) patterns.starts_with_think++;
  if (trimmed.includes('"classification"')) patterns.contains_classification++;
  if (trimmed.includes("tool_call") || trimmed.includes("tool_name") || trimmed.includes("json_filter") || trimmed.includes("regex_search")) patterns.contains_tool_call++;
  if (trimmed.includes("unsupported")) patterns.contains_unsupported++;
  if (trimmed.includes("summary") || trimmed.includes("Summary")) patterns.contains_summary++;
  if (trimmed.includes("```") || trimmed.includes("##")) patterns.contains_markdown++;
}
...
// Show unique error messages from planner_invalid_response events
...
// Check what NO_REASON failures look like (141 of them)
