...
- Redundant-E2E trim attempted via a rigorous coverage-diff pass (2026-06-18): an unshipped per-test branch-attribution harness (`scripts/analysis/`, c8 isolation per `node:test` case) attributed all 7 in-scope suites and surfaced near-subset pairs. Outcome: **no safe deletions**. The pairwise branch overlap is structural — every case re-covers the same boot/import branches — so a residual-≤8 branch-subset does not imply behavioral redundancy. Each surfaced candidate was either an unrelated case sharing setup branches or a deliberate two-sided/data-variation test (e.g. reachable vs unreachable endpoint, `--no-ignore` vs `-u` rg flag, thinking vs non-thinking headroom) whose each side asserts a distinct branch. The harness is retained as a reusable maintenance utility; the seam-rebalancing below remains the open work.
- Timing-sensitive tests hardened against load (2026-06-18): `tests/model-request-queue.test.ts` previously drove queue timeouts with real ~25–60ms `setTimeout` windows whose relative ordering broke under load. The queue-timeout cases now use deterministic virtual time (`node:test` mock timers + `tick()`), and the queue-state cases drop their arbitrary observation sleeps (enqueue/grant/release are synchronous), so the file no longer depends on the wall clock at all — verified 0/32 failures at 24-core oversubscription. The managed-llama startup/idle tests gate on condition-based `waitForAsyncExpectation` (5s); their residual fixed sleeps are documented stability/non-event assertions (asserting a value holds across the lease window — not pollable). One unrelated intermittent remains open: `runtime-planner-mode` "planner retries malformed json_filter schema-placeholder args" occasionally finds zero planner-debug dumps under full-suite load — not yet root-caused or reproduced in isolation.
