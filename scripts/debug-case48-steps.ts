...
const _distRoot = path.resolve(__dirname, '..', 'dist');
const _distExists = fs.existsSync(path.join(_distRoot, 'config', 'index.js'));
const _base = _distExists ? _distRoot : path.resolve(__dirname, '..');
const {
  loadConfig,
...
} = require(path.join(_base, 'config', 'index.js'));
const { acquireExecutionLock, releaseExecutionLock } = require(path.join(_base, 'execution-lock.js'));
const { buildPrompt } = require(path.join(_base, 'summary.js'));
const { countLlamaCppTokens, generateLlamaCppResponse } = require(path.join(_base, 'providers', 'llama-cpp.js'));

type SiftConfig = Record<string, unknown>;
...
function createLogger(logPath: string): (message: string) => void {
...
function withDeadline<T>(label: string, timeoutMs: number, operation: () => T | Promise<T>): Promise<DeadlineResult<T>> {
...
function getTokenAwareChunkThreshold(inputLength: number, promptTokenCount: number, effectivePromptLimit: number): number | null {
  if (
    inputLength <= 1
...
function tryParseDecision(text: string): Record<string, unknown> | null {
  try {
...
async function timedStep<T>(log: (msg: string) => void, label: string, timeoutMs: number, operation: () => T | Promise<T>): Promise<T> {
  log(`START ${label}`);
...
async function countPromptTokens(log: (msg: string) => void, options: CountPromptOptions): Promise<{ prompt: string; tokenCount: number | null }> {
...
async function inspectChunkPlanning(log: (msg: string) => void, options: InspectOptions): Promise<string[]> {
  const chunks: string[] = [];
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const manifest = await timedStep(log, 'read fixture manifest', timeoutMs, () => JSON.parse(fs.readFileSync(manifestPath, 'utf8')) as Fixture[]);
    ...
    const chunkResponses: Record<string, unknown>[] = [];
