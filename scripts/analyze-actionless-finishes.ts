...
interface ActionlessCase {
  ...
  isFakeInput: boolean;
  looksLikeRealSummary: boolean;
  thinkingProcess: string | undefined;
}
...
const actionlessCases: ActionlessCase[] = [];

for (const file of debugFiles) {
  let data: PlannerDebugData;
  try {
    data = JSON.parse(fs.readFileSync(path.join(LOGS_DIR, file), 'utf8')) as PlannerDebugData;
  } catch {
    continue;
  }
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    let modelResponse: PlannerEvent | null = null;
    for (let j = i - 1; j >= 0; j--) {
    ...
    try {
      parsed = JSON.parse(modelResponse.responseText ?? '') as ParsedResponse;
    } catch {
      continue;
    }
    ...
    let originalQuestion: string | null = data.question || data.command || null;
    const inputTextPreview = data.inputText
    ...
    if (!originalQuestion) {
      ...
      if (fs.existsSync(reqFile)) {
        try {
          const reqData = JSON.parse(fs.readFileSync(reqFile, 'utf8')) as Record<string, unknown>;
          originalQuestion = (reqData.question as string) || (reqData.command as string) || '(not found)';
        } catch { /* ignore */ }
      }
    }

    const isTestFiller = data.inputText ? /^A{100,}/.test(data.inputText.trim()) : false;
    const isJsonFiller = data.inputText ? /^\{"system_prompt":"A{50,}/.test(data.inputText.trim()) : false;
...
for (const [cls, count] of Object.entries(classificationCounts).sort((a, b) => (b[1] as number) - (a[1] as number))) {
...
for (const c of actionlessCases) {
  ...
  uniqueOutputs.set(key, (uniqueOutputs.get(key) || 0) + 1);
}
...
const selected: ActionlessCase[] = [];
const usedKeys = new Set<string>();

function addCase(c: ActionlessCase): boolean {
...
const fakeByOutput = new Map<string, ActionlessCase[]>();
