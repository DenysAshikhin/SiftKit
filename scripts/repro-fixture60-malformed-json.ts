...
} = require(path.join(_base, 'summary.js'));
...
interface RunManifest {
  ...
  malformedChunk: MalformedChunkInfo | null;
  chunks: ChunkRecord[];
  malformedFixture: { fixtureIndex: number; fixtureName: string; sourcePath: string; chunkPath: string; error: string } | null;
...
function createLogger(logPath: string, stdoutTarget: NodeJS.WritableStream = process.stdout): { log: (message: string) => void } {
...
export function resolveWorkItems(fixtureRoot: string, fixtureStartIndex: number, fixtureEndIndex: number): WorkItem[] {
...
function getLlamaCppPromptTokenReserve(config: SiftConfig): number {
  const reasoning = getConfiguredLlamaSetting<string>(config, 'Reasoning');
...
function getFixtureBounds(args: ParsedArgs): { fixtureStartIndex: number; fixtureEndIndex: number } {
...
function buildFixtureManifest(workItem: WorkItem, backend: string, model: string, requestTimeoutSeconds: number): FixtureManifest {
  return {
    ok: false,
...
function mirrorFixtureSummary(manifest: RunManifest, fixtureManifest: FixtureManifest): void {
...
export async function runFixture60MalformedJsonRepro(argv: string[], options: RunOptions = {}): Promise<RunResult> {
...
const isMainModule = process.argv[1] && (
  process.argv[1].endsWith('repro-fixture60-malformed-json.ts') ||
