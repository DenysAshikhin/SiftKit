...
interface PlannerDebugData {
  events?: PlannerEvent[];
  final?: { reason?: string; status?: string; finalOutput?: string; providerError?: string };
  [key: string]: unknown;
}
...
function loadDebug(plannerDebugPath: string): PlannerDebugData | null {
  const basename = path.basename(plannerDebugPath);
  try {
    return JSON.parse(fs.readFileSync(LOGS_DIR + basename, 'utf8')) as PlannerDebugData;
  } catch {
    return null;
  }
}
...
const allRequests: RequestData[] = files.map(f => {
  const data = JSON.parse(fs.readFileSync(REQUESTS_DIR + f, 'utf8')) as RequestData;
  data._filename = f;
  return data;
});
...
for (const req of failures) {
  ...
  const entry: CategoryEntry = { req, debug, reason };
...
const successWithEvents: { req: RequestData; debug: PlannerDebugData }[] = [];
