...
function printFull(label, value) {
  console.log(`\n  [${label}]:`);
...
const jsonFilterFailures = []; // tool events where json_filter had an error
const regexFailures = []; // tool events where regex was invalid
const allInvalidEvents = []; // every invalid response event for summary
...
for (const fileName of allFiles) {
  ...
  try {
    data = JSON.parse(
      fs.readFileSync(path.join(LOG_DIR, fileName), "utf8")
    );
  } catch {
    continue;
  }
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    // ── collect planner_invalid_response events ──
    if (ev.kind === "planner_invalid_response") {
      ...
      // find preceding model response
      let precedingModelText = null;
      let precedingToolCall = null;
      for (let j = i - 1; j >= 0; j--) {
        if (events[j].kind === "planner_model_response") {
          precedingModelText = events[j].responseText;
          break;
        }
      }

      // attempt to parse the model response to see if a tool call was attempted
      if (precedingModelText) {
        try {
          const parsed = JSON.parse(precedingModelText);
    ...
    // ── collect json_filter tool-level failures ──
    if (ev.kind === "planner_tool" && ev.toolName === "json_filter") {
      ...
      if (
        ...
        // Only if it looks like a real error, not just data containing the word
        if (
          outError ||
    ...
    // ── collect regex failures (find_text with invalid regex) ──
    ...
    // Also check if output object itself has an error field for find_text
...
for (const [key, cat] of Object.entries(categories)) {
  ...
  // Deduplicate by raw model text to show unique failure patterns
  ...
  let shown = 0;
  for (const [text, { item, count }] of uniqueTexts) {
...
separator(
  `json_filter TOOL-LEVEL FAILURES  (${jsonFilterFailures.length} occurrences)`
);

if (jsonFilterFailures.length === 0) {
  console.log("\n  No json_filter tool-level failures found.");
  console.log(
    "  Note: json_filter validation errors appear as planner_invalid_response events instead."
  );
...
if (jsonFilterInvalids.length > 0) {
  separator(
    `json_filter VALIDATION ERRORS (via planner_invalid_response)  (${jsonFilterInvalids.length} occurrences)`
  );

  const uniqueJsonFilter = new Map();
  ...
  let shown = 0;
  for (const [text, { item, count }] of uniqueJsonFilter) {
    if (shown >= MAX_EXAMPLES_PER_CATEGORY) break;
    shown++;

    subSeparator(
      `json_filter validation error ${shown} (appears ${count}x) — File: ${item.file}`
    );
...
if (regexFailures.length === 0) {
  ...
  const uniqueRegex = new Map();
  for (const item of regexFailures) {
    const key = (item.args?.query || "") + "|" + (item.outputText || item.outputError || "");
    if (!uniqueRegex.has(key)) {
      uniqueRegex.set(key, { item, count: 1 });
    } else {
      uniqueRegex.get(key).count++;
    }
  }
  ...
  let shown = 0;
  for (const [key, { item, count }] of uniqueRegex) {
    if (shown >= MAX_EXAMPLES_PER_CATEGORY) {
      console.log(
    ...
    subSeparator(
      `Regex failure ${shown} (appears ${count}x) — File: ${item.file}`
    );
