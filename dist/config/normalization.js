...
exports.applyRuntimeCompatibilityView = applyRuntimeCompatibilityView;
exports.updateRuntimePaths = updateRuntimePaths;
exports.toPersistedConfigObject = toPersistedConfigObject;
exports.normalizeConfig = normalizeConfig;
const paths_js_1 = require("../lib/paths.js");
const constants_js_1 = require("./constants.js");
const defaults_js_1 = require("./defaults.js");
const paths_js_2 = require("./paths.js");
function isLegacyManagedStartupScriptPath(value) {
    if (typeof value !== 'string' || !value.trim()) {
        return false;
    }
    const normalized = (0, paths_js_1.normalizeWindowsPath)(value.trim());
    return normalized === (0, paths_js_1.normalizeWindowsPath)(constants_js_1.SIFT_PREVIOUS_DEFAULT_LLAMA_STARTUP_SCRIPT)
        || normalized === (0, paths_js_1.normalizeWindowsPath)(constants_js_1.SIFT_FORMER_DEFAULT_LLAMA_STARTUP_SCRIPT)
        || normalized === (0, paths_js_1.normalizeWindowsPath)(constants_js_1.SIFT_BROKEN_DEFAULT_LLAMA_STARTUP_SCRIPT);
}
function applyRuntimeCompatibilityView(config) {
    const defaults = (0, defaults_js_1.getDefaultConfigObject)();
    ...
    return {
        ...
        PromptPrefix: config.PromptPrefix ?? constants_js_1.SIFT_DEFAULT_PROMPT_PREFIX,
...
function updateRuntimePaths(config) {
    return {
        ...config,
        Paths: (0, paths_js_2.initializeRuntime)(),
    };
}
...
function normalizeConfig(config) {
    ...
    for (const key of constants_js_1.RUNTIME_OWNED_LLAMA_CPP_KEYS) {
    ...
    if (updated.Runtime.Model === constants_js_1.SIFT_PREVIOUS_DEFAULT_MODEL) {
        updated.Runtime.Model = null;
        changed = true;
    }
    const numCtx = Number(updated.Runtime.LlamaCpp.NumCtx);
    const isLegacyDefaultSettings = (numCtx === constants_js_1.SIFT_LEGACY_DEFAULT_NUM_CTX
        && (!hadExplicitMaxInputCharacters || legacyMaxInputCharactersValue === constants_js_1.SIFT_LEGACY_DEFAULT_MAX_INPUT_CHARACTERS));
    const isLegacyDerivedSettings = (numCtx === constants_js_1.SIFT_LEGACY_DERIVED_NUM_CTX
        && !hadExplicitMaxInputCharacters);
    const isPreviousDefaultSettings = (numCtx === constants_js_1.SIFT_PREVIOUS_DEFAULT_NUM_CTX
        && !hadExplicitMaxInputCharacters);
