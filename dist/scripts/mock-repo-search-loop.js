#!/usr/bin/env node
"use strict";
// @ts-nocheck — Full typing deferred; mechanical CJS → ESM conversion for CP8.5.
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TASK_PACK = void 0;
...
exports.assertConfiguredModelPresent = assertConfiguredModelPresent;
exports.runMockRepoSearch = runMockRepoSearch;
exports.resolveRepoSearchRequestMaxTokens = resolveRepoSearchRequestMaxTokens;
exports.estimateTokenCount = estimateTokenCount;
exports.countTokensWithFallback = countTokensWithFallback;
exports.preflightPlannerPromptBudget = preflightPlannerPromptBudget;
exports.compactPlannerMessagesOnce = compactPlannerMessagesOnce;
...
const { loadConfig, getConfiguredModel, getConfiguredLlamaBaseUrl, getConfiguredLlamaNumCtx, getConfiguredLlamaSetting, getEffectiveInputCharactersPerContextToken, } = require(node_path_1.default.join(_base, 'config', 'index.js'));
const { listLlamaCppModels, countLlamaCppTokens } = require(node_path_1.default.join(_base, 'providers', 'llama-cpp.js'));
...
function createJsonlLogger(filePath) {
    ...
    node_fs_1.default.writeFileSync(target, '', 'utf8');
    return {
        path: target,
        write(event) {
            node_fs_1.default.appendFileSync(target, `${JSON.stringify({ at: new Date().toISOString(), ...event })}\n`, 'utf8');
        },
    };
}
/**
        ...
        const request = transport.request({
            ...
            if ((response.statusCode || 0) >= 400) {
                ...
                response.on('end', () => {
                    if (!settled) {
                        settled = true;
                        const serialized = serializeNetworkError(new Error(`llama.cpp planner stream failed with HTTP ${response.statusCode}${body.trim() ? `: ${body.trim().slice(0, 400)}` : '.'}`));
                        options.logger?.write({
                            kind: 'provider_request_error',
                            stage,
                            method,
            ...
            response.on('end', () => {
                ...
                if (toolCalls.length > 0 && toolCalls[0].name === 'run_repo_cmd') {
                    let args;
                    try {
                        args = JSON.parse(toolCalls[0].arguments);
                    }
                    catch {
                        args = null;
                    }
...
function parsePlannerAction(text) {
    ...
    if (action === 'tool') {
        if (parsed.tool_name !== 'run_repo_cmd'
            || !parsed.args
...
function collectOptionPaths(tokens, optionConfig) {
    ...
    const { valueOptions, pathOptions, positionalArePaths, rgPatternOptions, } = config;
    let index = 1;
...
function evaluateCommandSafety(command, repoRoot = '') {
    ...
    for (const segment of segments) {
        ...
        if (/\bforeach-object\b/iu.test(segment)
...
function rewriteRgWithFixedStrings(commandStr, pattern) {
    ...
    const withoutPattern = commandStr.replace(new RegExp(`(["'])${pattern.replace(/[.*+?^${}()|[\]\\]/gu, '\\$&')}\\1`), '').replace(/^(rg)\s+/iu, `$1 -F ${quotedAlternatives} `).replace(/\s{2,}/gu, ' ').trim();
    ...
    if (repoRoot) {
        ...
        try {
            if (node_fs_1.default.existsSync(gitignorePath)) {
                const lines = node_fs_1.default.readFileSync(gitignorePath, 'utf8').split(/\r?\n/gu);
...
function executeRepoCommand(command, repoRoot, mockCommandResults) {
    ...
    return new Promise((resolve) => {
        const child = (0, node_child_process_1.spawn)('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', command], {
            cwd: repoRoot,
            windowsHide: true,
...
function readAgentsMd(repoRoot) {
    if (!repoRoot)
        return '';
    const agentsPath = node_path_1.default.join(repoRoot, 'agents.md');
    try {
        if (node_fs_1.default.existsSync(agentsPath)) {
            const content = node_fs_1.default.readFileSync(agentsPath, 'utf8').trim();
            if (content)
                return content;
        }
    }
...
async function runTaskLoop(task, options) {
    ...
    for (let turn = 1; turn <= maxTurns; turn += 1) {
        ...
        if (!preflight.ok) {
            const overflowError = new Error(`planner_preflight_overflow prompt_tokens=${preflight.promptTokenCount} `
...
function parseCliArgs(argv) {
    const parsed = {
        ...
        logFile: node_path_1.default.join(node_os_1.default.tmpdir(), `siftkit-mock-repo-search-${Date.now()}.jsonl`),
    };
...
async function main() {
    ...
    process.stdout.write(`Summary: verdict=${scorecard.verdict} passed=${scorecard.totals.passed}/${scorecard.totals.tasks} `
...
const isMainModule = process.argv[1] && (process.argv[1].endsWith('mock-repo-search-loop.ts') ||
    process.argv[1].endsWith('mock-repo-search-loop.js'));
if (isMainModule) {
    main().catch((error) => {
        const message = error instanceof Error ? error.message : String(error);
