#!/usr/bin/env node
"use strict";
// @ts-nocheck — Ad-hoc debug harness; full typing not practical for fixture-driven scripts.
...
const { loadConfig, getChunkThresholdCharacters, getConfiguredLlamaNumCtx, getConfiguredModel, getConfiguredPromptPrefix, } = require(node_path_1.default.join(_base, 'config', 'index.js'));
const { acquireExecutionLock, releaseExecutionLock } = require(node_path_1.default.join(_base, 'execution-lock.js'));
const { buildPrompt } = require(node_path_1.default.join(_base, 'summary.js'));
...
function getTokenAwareChunkThreshold(inputLength, promptTokenCount, effectivePromptLimit) {
    ...
    const scaledThreshold = Math.floor(inputLength * (effectivePromptLimit / promptTokenCount) * 0.95);
...
async function countPromptTokens(log, options) {
    ...
    const tokenCount = await timedStep(log, `tokenize prompt ${options.label} chars=${prompt.length}`, options.timeoutMs, () => countLlamaCppTokens(options.config, prompt));
...
async function inspectChunkPlanning(log, options) {
    ...
    while (offset < options.inputText.length) {
        ...
        while (candidateLength > 0 && adjustmentCount < MAX_TOKEN_AWARE_CHUNK_ADJUSTMENTS) {
            ...
            log(`INFO  chunkCandidate offset=${offset} len=${candidateLength} attempt=${adjustmentCount} `
            ...
            if (tokenCount <= effectivePromptLimit) {
                ...
                const grownLength = Math.min(remainingLength, Math.max(acceptedLength + 1, Math.floor(acceptedLength * (effectivePromptLimit / Math.max(tokenCount, 1)))));
            ...
            if (reducedLength === null || reducedLength >= candidateLength) {
                throw new Error(`Unable to shrink chunk at offset=${offset}; candidateLength=${candidateLength} promptTokens=${tokenCount}.`);
            }
...
async function main() {
    ...
    const fixtureRoot = node_path_1.default.join(repoRoot, 'eval', 'fixtures', 'ai_core_60_tests');
    const manifestPath = node_path_1.default.join(fixtureRoot, 'fixtures.json');
    const logPath = node_path_1.default.join(repoRoot, 'tmp-find', `case${args.fixtureIndex}_step_trace_${getTimestamp()}.log`);
    ...
    try {
        ...
        const manifest = await timedStep(log, 'read fixture manifest', timeoutMs, () => JSON.parse(node_fs_1.default.readFileSync(manifestPath, 'utf8')));
        const fixture = manifest[args.fixtureIndex - 1];
        if (!fixture) {
            throw new Error(`Fixture ${args.fixtureIndex} not found in ${manifestPath}.`);
        }
        const fixturePath = node_path_1.default.join(fixtureRoot, fixture.File);
        const inputText = await timedStep(log, `read fixture input ${node_path_1.default.basename(fixturePath)}`, timeoutMs, () => node_fs_1.default.readFileSync(fixturePath, 'utf8'));
        ...
        const fullPrompt = await timedStep(log, 'build full prompt', timeoutMs, () => buildPrompt({
        ...
        const fullPromptTokens = await timedStep(log, 'tokenize full prompt', timeoutMs, () => countLlamaCppTokens(config, fullPrompt));
        ...
        for (let index = 0; index < chunks.length; index += 1) {
            const chunkPrompt = await timedStep(log, `build chunk ${index + 1} prompt`, timeoutMs, () => buildPrompt({
                question: fixture.Question,
            ...
            const chunkResponse = await timedStep(log, `provider chunk ${index + 1}/${chunks.length} request`, timeoutMs, () => generateLlamaCppResponse({
                config,
                model,
                prompt: chunkPrompt,
        ...
        const mergePrompt = await timedStep(log, 'build merge prompt', timeoutMs, () => buildPrompt({
        ...
        const mergeResponse = await timedStep(log, 'provider merge request', timeoutMs, () => generateLlamaCppResponse({
            config,
            model,
            prompt: mergePrompt,
            timeoutSeconds: args.requestTimeoutSeconds,
