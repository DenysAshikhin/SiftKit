...
const LOGS_DIR = node_path_1.default.join(__dirname, '..', '.siftkit', 'logs');
const REQUESTS_DIR = node_path_1.default.join(LOGS_DIR, 'requests');
const debugFiles = node_fs_1.default.readdirSync(LOGS_DIR)
...
for (const file of debugFiles) {
    let data;
    try {
        data = JSON.parse(node_fs_1.default.readFileSync(node_path_1.default.join(LOGS_DIR, file), 'utf8'));
    }
    catch {
        continue;
    }
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (!('classification' in parsed) || ('action' in parsed))
            continue;
        let toolCallCount = 0;
        for (let j = 0; j < i; j++) {
            if (events[j].kind === 'planner_tool')
                toolCallCount++;
        }
        ...
        const inputTextPreview = data.inputText
            ...
            : null;
        if (!originalQuestion) {
            const reqFile = node_path_1.default.join(REQUESTS_DIR, `request_${data.requestId}.json`);
            if (node_fs_1.default.existsSync(reqFile)) {
                try {
                    const reqData = JSON.parse(node_fs_1.default.readFileSync(reqFile, 'utf8'));
...
for (const c of fakeCases) {
    ...
    if (!fakeByOutput.has(key))
        fakeByOutput.set(key, []);
