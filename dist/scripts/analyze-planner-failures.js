...
const LOG_DIR = node_path_1.default.join(__dirname, '..', '.siftkit', 'logs');
...
const allFiles = node_fs_1.default
    .readdirSync(LOG_DIR)
...
for (const fileName of allFiles) {
    let data;
    try {
        data = JSON.parse(node_fs_1.default.readFileSync(node_path_1.default.join(LOG_DIR, fileName), 'utf8'));
    }
    catch {
        continue;
    }
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        if (ev.kind === 'planner_tool' && ev.toolName === 'find_text') {
            const outText = ev.output?.text || '';
            const outError = ev.output?.error;
            if (outText.includes('invalid regex') ||
        ...
        if (ev.kind === 'planner_tool' && ev.toolName === 'find_text' && ev.output?.error) {
            const already = regexFailures.some((r) => r.file === fileName && r.args?.query === ev.args?.query);
            if (!already) {
                regexFailures.push({
                    file: fileName,
                    toolName: ev.toolName,
...
if (jsonFilterFailures.length === 0) {
    ...
    for (let i = 0; i < shown.length; i++) {
        ...
        if (ex.command)
            printFull('Raw command string', ex.command);
    }
}
...
if (regexFailures.length === 0) {
    ...
    for (const [, { item, count }] of uniqueRegex) {
        ...
        if (item.command)
            console.log(`  Raw command: ${item.command}`);
        if (item.question)
            console.log(`  Original question: ${item.question}`);
    }
}
...
for (const fileName of allFiles) {
    ...
    if (data.final?.reason === 'planner_invalid_response_limit' ||
        data.final?.status === 'failed') {
        const invalidCount = (data.events || []).filter((e) => e.kind === 'planner_invalid_response').length;
        if (invalidCount > 0) {
            limitFailCount++;
        }
    }
