...
test('runTaskLoop truncates oversized rg output to the largest fitting prefix', async () => {
  ...
  const budget = new TurnBudget({ compactionReserveTokens: PROMPT_COMPACTION_RESERVE_TOKENS, totalContextTokens, maxTurns: 3 });
...
test('runTaskLoop increases per-tool cap as tool-call progress grows', async () => {
  const events: JsonObject[] = [];
  const totalContextTokens = 20000;
  const budget = new TurnBudget({ compactionReserveTokens: PROMPT_COMPACTION_RESERVE_TOKENS, totalContextTokens, maxTurns: 10 });
  const baselinePerToolCapTokens = budget.perToolCapTokens(0, 1);
...
function historyCharsForPromptLimit(taskKind: LoopTaskKind, totalContextTokens: number): number {
  const budget = new TurnBudget({ compactionReserveTokens: PROMPT_COMPACTION_RESERVE_TOKENS, totalContextTokens, maxTurns: 6 });
