  ...
  const result = await runTaskLoop(
    { id: 'agent-forced', question: 'Do the task.', signals: [] },
    {
      ...
      mockResponses: [
        { toolCalls: [{ name: "git", arguments: {"operation":"grep","pattern":"planner","path":"src"} }] },
        { content: "v1" },
        { toolCalls: [{ name: "git", arguments: {"operation":"grep","pattern":"budget","path":"src"} }] },
        { content: "v2" },
        { toolCalls: [{ name: "git", arguments: {"operation":"grep","pattern":"tokens","path":"src"} }] },
        { content: "v3" },
        { content: '{"verdict":"pass","reason":"supported"}' },
      ],
      mockCommandResults: {
    ...
    { id: 'agent-forced-mode', question: 'Do the task.', signals: [] },
    {
      ...
      mockResponses: [
        ...patterns.map((pattern) => ({
          toolCalls: [{ name: 'git', arguments: { operation: 'grep', pattern, path: 'src' } }],
        })),
