...
const { makeCaptureStream, withTestEnvAndServer } = require('./_test-helpers.js');
...
test('internal op config-get returns config object', async () => {
  await withTestEnvAndServer(async ({ tempRoot }) => {
    ...
    assert.equal(code, 0);
    const parsed = JSON.parse(stdout.read().trim());
...
test('--prompt shortcut triggers repo-search', async () => {
  await withTestEnvAndServer(async () => {
    ...
    // repo-search routes through the status server's /repo-search endpoint
    // which our stub doesn't implement, so this will error, but it proves
    // the command routing works (it reaches the server health check first)
