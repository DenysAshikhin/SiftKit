...
test('repo-agent --approval off runs autonomously (non-TTY) and applies a mutation', async () => {
  ...
  try {
    ...
    const code = await runCli({
      argv: ['repo-agent', '--prompt', 'make x', '--approval', 'off'],
    ...
    assert.equal(received[0].approval, 'off');
...
test('repo-agent default (interactive) on a non-TTY stdin fails fast with an approval-TTY error', async () => {
  const hits: string[] = [];
  const server = http.createServer((req, res) => {
