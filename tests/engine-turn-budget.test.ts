...
test('TurnBudget exposes the shared compaction reserve and prompt limit', () => {
  const budget = new TurnBudget({ compactionReserveTokens: PROMPT_COMPACTION_RESERVE_TOKENS, totalContextTokens: 155_000, maxTurns: 45 });
  assert.equal(
...
test('TurnBudget clamps the reserve to half of a small context', () => {
  const budget = new TurnBudget({ compactionReserveTokens: PROMPT_COMPACTION_RESERVE_TOKENS, totalContextTokens: 8_000, maxTurns: 45 });
  assert.equal(budget.compactionReserveTokens, 4_000);
  assert.equal(budget.maxPromptTokens, 4_000);
});

test('TurnBudget keeps the fixed reserve for a large context', () => {
  const budget = new TurnBudget({ compactionReserveTokens: PROMPT_COMPACTION_RESERVE_TOKENS, totalContextTokens: 140_000, maxTurns: 45 });
  assert.equal(budget.compactionReserveTokens, 15_000);
  assert.equal(budget.maxPromptTokens, 125_000);
});

test('maxPromptTokens never goes negative', () => {
  const budget = new TurnBudget({ compactionReserveTokens: PROMPT_COMPACTION_RESERVE_TOKENS, totalContextTokens: 1, maxTurns: 45 });
  assert.equal(budget.compactionReserveTokens, 1);
  assert.equal(budget.maxPromptTokens, 0);
});

test('a lone tool call early in a run gets the whole floor share', () => {
  const budget = new TurnBudget({ compactionReserveTokens: PROMPT_COMPACTION_RESERVE_TOKENS, totalContextTokens: 100_000, maxTurns: 45 });
...
test('the turn share still grows with completed tool-call progress', () => {
  const budget = new TurnBudget({ compactionReserveTokens: PROMPT_COMPACTION_RESERVE_TOKENS, totalContextTokens: 100_000, maxTurns: 10 });
...
test('TurnBudget rejects an invalid context window instead of clamping it', () => {
  assert.throws(
    () => new TurnBudget({ compactionReserveTokens: PROMPT_COMPACTION_RESERVE_TOKENS, totalContextTokens: -10, maxTurns: 0 }),
