...
class CountingClient extends HttpClient {
  calls = 0;

  override fetch(url: string | URL): Promise<Response> {
    this.calls += 1;
...
class FakeHttpClient extends HttpClient {
  constructor(private readonly fetchImpl: (url: string | URL) => Promise<Response>) {
    super();
  }
...
test('readWebSearchQuotas returns one quota per active provider', async () => {
  ...
  const quotas = await readWebSearchQuotas(makeConfig(), client);
...
test('readWebSearchQuotas degrades to nulls when a provider errors', async () => {
  const client = new FakeHttpClient(async (url) => String(url).includes('tavily')
    ? jsonResponse({}, 500)
...
test('readWebSearchQuotas returns empty array when no provider active', async () => {
  ...
  const quotas = await readWebSearchQuotas(config, new FakeHttpClient(async () => jsonResponse({})));
  assert.deepEqual(quotas, []);
});
...
test('WebSearchQuotaCache reuses results within the TTL (no refetch)', async () => {
  ...
  const client = new CountingClient();
  const first = await cache.read(makeConfig(), client);
  const second = await cache.read(makeConfig(), client);
...
test('WebSearchQuotaCache refetches once the entry has expired (ttl 0)', async () => {
  ...
  const client = new CountingClient();
  await cache.read(makeConfig(), client);
  await cache.read(makeConfig(), client);
...
test('WebSearchQuotaCache keys on provider config so key changes bypass the cache', async () => {
  ...
  await cache.read(changed, client);
