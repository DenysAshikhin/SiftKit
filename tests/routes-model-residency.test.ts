...
async function startManagedEngineServer(name: string, gpuMemoryProbe?: GpuMemoryProbe) {
  ...
  const server = await DashboardTestServer.start(
    ...
    { managedEngineStartup: true, gpuMemoryProbe },
  );
  return { server, stub };
}
...
test('GET /runtime/inference reports free VRAM from the GPU memory probe', async () => {
  const { server, stub } = await startManagedEngineServer(
    'residency-gpu-',
    new FixedGpuMemoryProbe({ totalBytes: 24 * MIB, usedBytes: 20 * MIB, freeBytes: 4 * MIB }),
  );
  try {
    const response = await requestJson(`${server.baseUrl}/runtime/inference`);
    assert.equal(response.statusCode, 200);
    assert.equal(response.body.gpuFreeBytes, 4 * MIB);
  } finally {
    await server.close();
    await stub.close();
  }
