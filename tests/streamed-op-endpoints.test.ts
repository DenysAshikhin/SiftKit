...
test('command-output/analyze streams progress and a schema-valid result', async (t) => {
  const harness = await startHarness('siftkit-streamed-cmd-', t);
...
test('preset/run streams a schema-valid result for a summary preset', async (t) => {
  const harness = await startHarness('siftkit-streamed-preset-', t);
...
test('eval/run answers over SSE with a terminal frame', async (t) => {
  const harness = await startHarness('siftkit-streamed-eval-', t);
