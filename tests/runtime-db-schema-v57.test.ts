...
/** A v56 database whose activity events still carry the single combined `idle_seconds`. */
function seedV56Database(): string {
  const dbPath = path.join(createManagedTempDir('sk-v57-'), 'runtime.sqlite');
  const database = new Database(dbPath);
  database.exec(`
    CREATE TABLE runtime_schema (id INTEGER PRIMARY KEY CHECK (id = 1), version INTEGER NOT NULL);
    INSERT INTO runtime_schema (id, version) VALUES (1, 56);
...
test('v57 splits the combined activity idle column into mouse and keyboard, seeding both', () => {
  const dbPath = seedV56Database();
  try {
