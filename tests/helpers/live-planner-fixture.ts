...
/** The active preset's model endpoint, interactive tool set, and its tools, or a loud failure. */
export async function loadLivePlannerFixture(): Promise<LivePlannerFixture> {
  ...
  return {
    ...
    baseUrl: requireConfiguredString(getConfiguredEngineBaseUrl(config), `active preset ${preset.id} has no configured base URL`),
