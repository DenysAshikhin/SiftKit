...
test('repo-search executes a native web_search tool when allowed', async () => {
  ...
  const scorecard = await runRepoSearch({
    ...
    mockResponses: [
      { toolCalls: [{ name: "web_search", arguments: {"query":"siftkit"} }] },
      { content: "done" },
    ],
    mockCommandResults: {
...
test('runTaskLoop reports prompt tokens and elapsed time on command progress events', async () => {
  ...
  assert.equal(toolStart?.command, "git operation=\"grep\" path=\"src\" pattern=\"planner\"");
  assert.equal(toolResult?.command, "git operation=\"grep\" path=\"src\" pattern=\"planner\"");
...
test('runTaskLoop tool_result outputTokens reflects the fitted bubble output', async () => {
  ...
  await runTaskLoop(
    ...
    },
    {
      ...
      mockCommandResults: {
        "git operation=\"grep\" path=\"src\" pattern=\"planner\"": { exitCode: 0, stdout: longStdout, stderr: '' },
      },
      progressWriter: new CollectingProgressWriter(progressEvents),
    }
  );
...
test('runTaskLoop does not replay final output as thinking progress', async () => {
  ...
  const result = await runTaskLoop(
    ...
    },
    {
      ...
      minToolCallsBeforeFinish: 0,
      mockResponses: [
        { content: "Duplicated answer body" },
        { content: '{"verdict":"pass","reason":"supported"}' },
      ],
      mockCommandResults: {},
      progressWriter: new CollectingProgressWriter(progressEvents),
          ...
          message: chatRequestCount === 1
            ? {
              ...
              tool_calls: [{
                id: 'status-1',
                type: 'function',
                function: { name: 'git', arguments: '{"operation":"status"}' },
              }],
            }
  ...
  try {
    const result = await runTaskLoop(
      ...
      },
      {
        ...
        minToolCallsBeforeFinish: 0,
        mockCommandResults: {
          "git operation=\"status\"": { exitCode: 0, stdout: ' M src/repo-search/engine.ts', stderr: '' },
        },
      }
    );
  ...
  try {
    ...
    const result = await runTaskLoop(
      ...
      },
      {
        ...
        minToolCallsBeforeFinish: 0,
        mockResponses: [
          { toolCalls: [{ name: "find", arguments: {"pattern":"*.ts","path":"src"} }] },
          { toolCalls: [{ name: "read", arguments: {"path":"src/sample.ts","offset":2,"limit":2} }] },
          { content: "done" },
          { content: '{"verdict":"pass","reason":"supported"}' },
        ],
        mockCommandResults: {},
...
test('runTaskLoop executes find with a runner-* glob natively', async () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(repoRoot, 'logs', 'runner-20260425.ndjson'), "{}", 'utf8');
    ...
    const result = await runTaskLoop(
      ...
      },
      {
        ...
        minToolCallsBeforeFinish: 0,
        mockResponses: [
          { toolCalls: [{ name: "find", arguments: {"pattern":"runner-*","path":"logs"} }] },
          { content: "done" },
          { content: '{"verdict":"pass","reason":"supported"}' },
        ],
        mockCommandResults: {},
...
test('runTaskLoop counts non-zero command exits as command failures without failing the run', async () => {
  const task = await runTaskLoop(
    ...
    },
    {
      ...
      mockCommandResults: {
        "git operation=\"grep\" path=\"src\" pattern=\"planner\"": { exitCode: 2, stdout: '', stderr: 'boom' },
      },
    }
  );
...
test('runTaskLoop counts exit code 1 from non-search commands as a command failure without failing the run', async () => {
  const task = await runTaskLoop(
    ...
    },
    {
      ...
      minToolCallsBeforeFinish: 0,
      mockResponses: [
        { toolCalls: [{ name: "git", arguments: {"operation":"log"} }] },
        { content: "failed" },
        { content: '{"verdict":"pass","reason":"ok"}' },
      ],
      mockCommandResults: {
        "git operation=\"log\"": { exitCode: 1, stdout: '', stderr: 'error' },
      },
    }
  );
...
test('runTaskLoop stops on finish action', async () => {
  const result = await runTaskLoop(
    ...
    },
    {
      ...
      minToolCallsBeforeFinish: 0,
      mockResponses: [
        { content: "find_text read_lines json_filter" },
        { content: '{"verdict":"pass","reason":"supported"}' },
      ],
      mockCommandResults: {},
    }
...
test('runTaskLoop executes tool batches sequentially and counts each tool call toward finish depth', async () => {
  ...
  assert.equal(result.commands[0].command.startsWith("git operation=\"grep\" path=\"src\" pattern=\"planner prompt\""), true);
  assert.equal(result.commands[1].command.startsWith("git operation=\"grep\" path=\"src\" pattern=\"prompt budget\""), true);
});
...
test('runTaskLoop accepts corroborated finish before minimum tool-call depth', async () => {
  ...
  const result = await runTaskLoop(
    ...
    },
    {
      ...
      mockResponses: [
        { toolCalls: [{ name: "git", arguments: {"operation":"grep","pattern":"planner","path":"src"} }] },
        { toolCalls: [{ name: "git", arguments: {"operation":"show","ref":"HEAD","path":"src/summary.ts"} }] },
        { content: "done" },
        { content: '{"verdict":"pass","reason":"supported"}' },
      ],
      mockCommandResults: {
        "git operation=\"grep\" path=\"src\" pattern=\"planner\"": { exitCode: 0, stdout: 'src\\summary.ts:10:planner hit', stderr: '' },
        "git operation=\"show\" ref=\"HEAD\" path=\"src/summary.ts\"": { exitCode: 0, stdout: 'src\\summary.ts:10: planner hit', stderr: '' },
      },
      logger: {
        path: 'memory',
        write(event) {
  ...
  try {
    ...
    const result = await runTaskLoop(
      ...
      },
      {
        ...
        minToolCallsBeforeFinish: 0,
        mockResponses: [
          { toolCalls: [{ name: "read", arguments: {"path":"summary.ts","offset":41,"limit":6} }] },
          { content: "done" },
          { content: '{"verdict":"pass","reason":"supported"}' },
        ],
        mockCommandResults: {},
              ...
              message: requestCount === 1
                ? {
                  ...
                  tool_calls: [{
                    ...
                    function: {
                      name: 'git',
                      arguments: '{"operation":"grep","pattern":"planner","path":"src"}',
                    },
                  }],
                }
                : { role: 'assistant', content: 'done' },
 ...
 */
const DUPLICATE_PATH_PADDING = 'x'.repeat(REJECTED_ARGS_ELISION_LIMIT);
const OVERSIZED_DUPLICATE_PATH = `src/${DUPLICATE_PATH_PADDING}`;
const OVERSIZED_DUPLICATE_COMMAND = `git operation="grep" path=${JSON.stringify(OVERSIZED_DUPLICATE_PATH)} pattern="planner"`;
      ...
      req.on('end', () => {
        ...
        sendChatCompletionSse(res, {
          ...
          choices: [
            {
              index: 0,
              message: requestCount <= 5
                ? {
                  role: 'assistant',
                  content: '',
                  tool_calls: [{
                    id: `duplicate-${requestCount}`,
                    type: 'function',
                    function: {
                      name: 'git',
...
test('runTaskLoop assigns a unique toolCallId pairing tool_start with tool_result', async () => {
  ...
  await runTaskLoop(
    ...
    },
    {
      ...
      mockResponses: [
        { toolCalls: [{ name: "git", arguments: {"operation":"grep","pattern":"planner","path":"src"} }] },
        { toolCalls: [{ name: "git", arguments: {"operation":"grep","pattern":"prompt","path":"src"} }] },
        { content: "done" },
        { content: '{"verdict":"pass","reason":"supported"}' },
      ],
      mockCommandResults: {
        "git operation=\"grep\" path=\"src\" pattern=\"planner\"": { exitCode: 0, stdout: 'planner hit', stderr: '' },
        "git operation=\"grep\" path=\"src\" pattern=\"prompt\"": { exitCode: 0, stdout: 'prompt hit', stderr: '' },
      },
      progressWriter: new CollectingProgressWriter(progressEvents),
    }
  );
