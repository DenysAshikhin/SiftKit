...
test('runtime cutover migration imports legacy files and deletes them', () => {
  withTempRepo((repoRoot) => {
    ...
    assert.equal(readObservedBudgetState().observedTelemetrySeen, false);
    assert.equal(readObservedBudgetState().lastKnownCharsPerToken, null);
    assert.equal(readObservedBudgetState().observedCharsTotal, null);
    assert.equal(readObservedBudgetState().observedTokensTotal, null);
...
test('runtime database initializes fresh app_config schema without re-adding v11 ncpu_moe columns', () => {
  withTempRepo(() => {
    ...
    assert.equal(Number(schemaVersion?.version || 0), 20);
  });
});
...
test('runtime database migrates schema v5 GPU fields to schema v6 boolean-only status storage', () => {
  withTempRepo(() => {
    ...
    assert.equal(versionRow.version, 20);
