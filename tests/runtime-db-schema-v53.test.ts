...
const LimitRowSchema = z.object({ tool_call_limit: z.number().nullable() });

test('v53 renames the persisted planner-turn field to the enforced tool-call limit', () => {
  const dbPath = path.join(createManagedTempDir('sk-v53-tool-limit-'), 'runtime.sqlite');
  try {
    ...
    database.prepare(`
      INSERT INTO chat_messages (
        ...
        tool_call_max_turns, created_at_utc, compressed_into_summary, position
      ) VALUES ('session-1', 'tool-1', 'assistant', 'assistant_tool_call', 'read', 0, 0, 0, 0, 0, 0, 45, ?, 0, 0)
    `).run(timestamp);
    database.prepare('UPDATE runtime_schema SET version = 52 WHERE id = 1').run();
    ...
    try {
      ...
      assert.equal(columns.some((column) => column.name === 'tool_call_limit'), true);
      assert.equal(columns.some((column) => column.name === 'tool_call_max_turns'), false);
      const row = LimitRowSchema.parse(readonly.prepare('SELECT tool_call_limit FROM chat_messages').get());
      assert.equal(row.tool_call_limit, 45);
