...
test('getConfiguredModel throws when model is missing', () => {
  assert.throws(
    () => getConfiguredModel({ LlamaCpp: {} }),
    /missing Model/u
  );
});

test('getConfiguredModel returns Runtime.Model', () => {
  assert.equal(getConfiguredModel({ Runtime: { Model: 'test-model' }, LlamaCpp: {} }), 'test-model');
});

test('getConfiguredLlamaBaseUrl throws when BaseUrl is missing', () => {
  assert.throws(
    () => getConfiguredLlamaBaseUrl({ LlamaCpp: {} }),
...
test('getConfiguredLlamaBaseUrl returns Runtime.LlamaCpp.BaseUrl', () => {
  assert.equal(
    getConfiguredLlamaBaseUrl({ Runtime: { LlamaCpp: { BaseUrl: 'http://test:8080' } }, LlamaCpp: {} }),
    'http://test:8080'
  );
});
...
test('getConfiguredLlamaNumCtx throws when NumCtx is missing', () => {
  assert.throws(
    () => getConfiguredLlamaNumCtx({ LlamaCpp: {} }),
...
test('getConfiguredLlamaNumCtx returns Runtime.LlamaCpp.NumCtx', () => {
  assert.equal(
    getConfiguredLlamaNumCtx({ Runtime: { LlamaCpp: { NumCtx: 65000 } }, LlamaCpp: {} }),
    65000
  );
});

test('getConfiguredPromptPrefix returns undefined for empty prefix', () => {
  assert.equal(getConfiguredPromptPrefix({ LlamaCpp: {} }), undefined);
  assert.equal(getConfiguredPromptPrefix({ PromptPrefix: '', LlamaCpp: {} }), undefined);
  assert.equal(getConfiguredPromptPrefix({ PromptPrefix: '   ', LlamaCpp: {} }), undefined);
});

test('getConfiguredPromptPrefix returns trimmed prefix', () => {
  assert.equal(getConfiguredPromptPrefix({ PromptPrefix: 'test prefix', LlamaCpp: {} }), 'test prefix');
});

test('getConfiguredLlamaSetting returns value from Runtime.LlamaCpp', () => {
  const config = { Runtime: { LlamaCpp: { Temperature: 0.5 } }, LlamaCpp: {} };
  assert.equal(getConfiguredLlamaSetting(config, 'Temperature'), 0.5);
});

test('getConfiguredLlamaSetting returns undefined for missing key', () => {
  const config = { Runtime: { LlamaCpp: {} }, LlamaCpp: {} };
  assert.equal(getConfiguredLlamaSetting(config, 'Temperature'), undefined);
});
...
test('getRepoLocalRuntimeRoot returns path when in SiftKit repo', () => {
  ...
  // This should return a path since we're running from within the SiftKit repo
...
test('loadConfig normalizes legacy previous-default model', async () => {
  const { SIFT_PREVIOUS_DEFAULT_MODEL } = require('../dist/config.js');
  await withTestEnvAndServer(async ({ stub }) => {
    stub.state.config.Runtime = stub.state.config.Runtime || {};
    stub.state.config.Runtime.Model = SIFT_PREVIOUS_DEFAULT_MODEL;
    const config = await loadConfig({ ensure: true });
    // After normalization, old default model should be replaced
    assert.notEqual(config.Runtime?.Model, SIFT_PREVIOUS_DEFAULT_MODEL);
  });
});

test('loadConfig normalizes legacy NumCtx settings', async () => {
  const { SIFT_LEGACY_DEFAULT_NUM_CTX } = require('../dist/config.js');
  await withTestEnvAndServer(async ({ stub }) => {
    stub.state.config.Runtime = stub.state.config.Runtime || {};
    stub.state.config.Runtime.LlamaCpp = stub.state.config.Runtime.LlamaCpp || {};
    stub.state.config.Runtime.LlamaCpp.NumCtx = SIFT_LEGACY_DEFAULT_NUM_CTX;
    const config = await loadConfig({ ensure: true });
    // After normalization, legacy NumCtx should be upgraded
...
test('loadConfig handles missing Interactive fields', async () => {
  await withTestEnvAndServer(async ({ stub }) => {
    delete stub.state.config.Interactive;
    const config = await loadConfig({ ensure: true });
...
test('loadConfig handles missing Server fields', async () => {
  await withTestEnvAndServer(async ({ stub }) => {
    delete stub.state.config.Server;
    const config = await loadConfig({ ensure: true });
...
test('loadConfig handles missing Thresholds fields', async () => {
  await withTestEnvAndServer(async ({ stub }) => {
    delete stub.state.config.Thresholds;
    const config = await loadConfig({ ensure: true });
...
test('loadConfig removes legacy MaxInputCharacters from Thresholds', async () => {
  await withTestEnvAndServer(async ({ stub }) => {
    stub.state.config.Thresholds = stub.state.config.Thresholds || {};
    stub.state.config.Thresholds.MaxInputCharacters = 50000;
    const config = await loadConfig({ ensure: true });
...
test('loadConfig removes legacy ChunkThresholdRatio from Thresholds', async () => {
  await withTestEnvAndServer(async ({ stub }) => {
    stub.state.config.Thresholds = stub.state.config.Thresholds || {};
    stub.state.config.Thresholds.ChunkThresholdRatio = 0.8;
    const config = await loadConfig({ ensure: true });
    assert.equal(config.Thresholds.ChunkThresholdRatio, undefined);
  });
});
...
test('loadConfig migrates top-level Model to Runtime.Model', async () => {
  await withTestEnvAndServer(async ({ stub }) => {
    ...
    stub.state.config.Runtime = stub.state.config.Runtime || {};
    delete stub.state.config.Runtime.Model;
    const config = await loadConfig({ ensure: true });
    // Model should have been migrated to Runtime.Model
    assert.equal(typeof config.Effective, 'object');
  });
});

test('loadConfig replaces legacy startup script paths', async () => {
  const { SIFT_PREVIOUS_DEFAULT_LLAMA_STARTUP_SCRIPT } = require('../dist/config.js');
  await withTestEnvAndServer(async ({ stub }) => {
    stub.state.config.Server = stub.state.config.Server || {};
    stub.state.config.Server.LlamaCpp = stub.state.config.Server.LlamaCpp || {};
    stub.state.config.Server.LlamaCpp.StartupScript = SIFT_PREVIOUS_DEFAULT_LLAMA_STARTUP_SCRIPT;
    const config = await loadConfig({ ensure: true });
