...
test('package files whitelist ships only runtime, not dev harnesses', () => {
  ...
  assert.ok(files.includes('scripts/postinstall.cjs'), 'postinstall hook script must be packed');
...
test('postinstall hook references the packed postinstall script', () => {
  ...
  assert.equal(scripts.postinstall, 'node scripts/postinstall.cjs');
});

test('coverage test scripts reuse the project test runner instead of raw discovery', () => {
  const { scripts } = readPackageJson();
  assert.match(scripts['test:coverage'], /\bdist[\\/]test-runner[\\/]run-tests\.js\b/u);
  assert.doesNotMatch(scripts['test:coverage'], /\btsx --test\b/u);
  assert.match(scripts['test:coverage:llm'], /\bdist[\\/]test-runner[\\/]run-tests\.js\b/u);
