...
test('requestJson logs repo-search client request lifecycle before server admission', async () => {
  ...
  try {
    ...
    assert.equal(lines.some((line) => /http_client enqueue_intent task=repo-search method=POST path=\/repo-search body_chars=2/u.test(line)), true, lines.join('\n'));
    assert.equal(lines.some((line) => /http_client request_start task=repo-search method=POST path=\/repo-search/u.test(line)), true, lines.join('\n'));
    assert.equal(lines.some((line) => /http_client request_sent task=repo-search method=POST path=\/repo-search elapsed_ms=\d+/u.test(line)), true, lines.join('\n'));
    assert.equal(lines.some((line) => /http_client response_received task=repo-search method=POST path=\/repo-search status=200 elapsed_ms=\d+/u.test(line)), true, lines.join('\n'));
    assert.equal(lines.some((line) => /http_client response_done task=repo-search method=POST path=\/repo-search status=200 response_chars=11 elapsed_ms=\d+/u.test(line)), true, lines.join('\n'));
  } finally {
    await new Promise<void>((resolve) => server.close(() => resolve()));
  }
});

test('requestJson logs summary client request lifecycle before server admission', async () => {
  const server = http.createServer((req, res) => {
