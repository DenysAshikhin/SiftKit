...
function makeCommand(command: string, turn: number, safe = true, exitCode = 0): TaskCommand {
  return { toolCallId: `tc_${turn}`, command, activityKind: 'command', activitySubject: { kind: 'none' }, turn, safe, reason: null, exitCode, output: '' };
}
...
test('marks entries failed for a missing exit code and for a non-zero exit code', () => {
  ...
  collector.recordBatch(
    ...
    [
      { toolCallId: 'tc_10a', command: 'killed', activityKind: 'command', activitySubject: { kind: 'none' }, turn: 10, safe: true, reason: null, exitCode: null, output: '' },
