...
const ALLOWED: DashboardOperationModeAllowedTools = {
  ...
  'read-only': ['find_text', 'read_lines', 'repo_rg'],
  full: ['find_text', 'read_lines', 'repo_rg', 'web_search'],
};
...
test('rows reflect per-mode membership', () => {
  ...
  const repoRg = groups[1]?.rows.find((row) => row.tool === 'repo_rg');
...
test('toggleToolInMode adds and removes via togglePresetTool', () => {
  const added = toggleToolInMode(ALLOWED, 'repo_rg', 'summary');
  assert.ok(added.includes('repo_rg'));
