...
test('load and restore is available for unloaded and frozen stable runtimes', () => {
  assert.equal(resolveResidencyControlState('unloaded', 'llama', false).load, true);
  assert.equal(resolveResidencyControlState('frozen', 'exl3', true).load, true);
  assert.equal(resolveResidencyControlState('ready', 'llama', false).load, false);
});
...
test('freeze to RAM is available only for a ready EXL3 runtime', () => {
  ...
  assert.equal(resolveResidencyControlState('ready', 'llama', false).freeze, false);
...
test('unload is available for ready and frozen stable resident runtimes', () => {
  assert.equal(resolveResidencyControlState('ready', 'llama', false).unload, true);
