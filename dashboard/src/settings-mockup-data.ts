...
export const SETTINGS_MOCKUP_SECTIONS: SettingsMockupSection[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { kind: 'text', label: 'Threads', value: '0', layout: 'quarter', description: 'CPU worker threads used for llama.cpp execution. Set `0` to omit `-t` and let llama.cpp choose its own default.' },
      { kind: 'toggle', label: 'Flash attention', value: true, layout: 'quarter' },
      { kind: 'text', label: 'ParallelSlots', value: '1', layout: 'quarter', description: 'Concurrent generation lanes; maps to llama.cpp parallel slots or EXL3 maximum batch size.' },
      ...
      { kind: 'text', label: 'UBatchSize', value: '512', layout: 'quarter', description: 'Prompt ingestion chunk size; maps to llama.cpp UBatch size or EXL3 chunk size.' },
      { kind: 'text', label: 'CacheRam', value: '8192', layout: 'quarter', description: 'Amount of RAM reserved for the llama.cpp KV cache, in MiB.' },
      { kind: 'text', label: 'KV cache quant', value: 'f16', layout: 'quarter', description: 'Applies `--cache-type-k` and `--cache-type-v` for the managed llama.cpp KV cache.' },
