...
export const SETTINGS_SECTIONS: Record<SettingsSectionId, SettingsSectionDescriptor> = {
  ...
  'model-presets': {
    ...
    fields: [
      ...
      { label: 'SpeculativeNgramSizeN', layout: 'quarter', helpText: 'Size of the matching n-gram window used to look up speculative continuations.' },
      { label: 'SpeculativeNgramSizeM', layout: 'quarter', helpText: 'Maximum speculative continuation length pulled from matching history.' },
      { label: 'SpeculativeNgramMinHits', layout: 'quarter', helpText: 'Minimum repeated matches required before `ngram-map-k` style speculation is used.' },
      { label: 'SpeculativeDraftMax', layout: 'quarter', helpText: 'Upper bound for draft tokens proposed in one speculative step.' },
      { label: 'SpeculativeDraftMin', layout: 'quarter', helpText: 'Minimum accepted draft token count before speculative decoding is used.' },
