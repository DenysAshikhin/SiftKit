...
export const SETTINGS_SECTIONS: Record<SettingsSectionId, SettingsSectionDescriptor> = {
  ...
  'model-presets': {
    ...
    fields: [
      ...
      { label: 'CacheRam', layout: 'quarter', helpText: 'Host RAM reserved for KV cache offload, in MiB; maps to the llama.cpp `--cache-ram` budget or the exllamav3 pinned second-tier page cache (`TABBY_MEMORY_SYSMEM_PAGE_CACHE`), which tensor parallel rejects.' },
