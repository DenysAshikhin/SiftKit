...
export const SETTINGS_SECTIONS: Record<SettingsSectionId, SettingsSectionDescriptor> = {
  ...
  'model-presets': {
    ...
    fields: [
      ...
      { label: 'GpuLayers', layout: 'quarter', helpText: 'Number of transformer layers placed on the GPU.' },
      ...
      { label: 'NcpuMoe', layout: 'quarter', helpText: 'Moves MoE expert weights off the GPU into system RAM, where the CPU computes them: higher frees more VRAM and costs speed, `0` keeps every expert on the GPU. The unit differs per backend. llama.cpp reads it as a LAYER count and passes `--n-cpu-moe`, offloading every expert of that many layers; `0` omits the flag. EXL3 reads it as an EXPERT-PER-LAYER count and passes `cpu_moe_split_experts`, so the coldest N of each MoE layer\x27s routed experts run on the CPU while the rest stay in VRAM — their GEMMs overlap the same layer\x27s GPU compute, and exllamav3 re-ranks hot/cold experts as it runs, so it costs far less speed than offloading whole layers. Must stay below the model\x27s expert count. Ignored on non-MoE models and on layers exllamav3 finds ineligible (non-mul1 experts, K > 8, mixed expert biases), which log a skip line at load.' },
      ...
      { label: 'SpeculativeDynamic', layout: 'quarter', helpText: 'Adapts each job\'s draft window from its acceptance rate, using SpeculativeDraftMax as the ceiling.' },
      ...
      { label: 'ReasoningBudget', layout: 'quarter', helpText: 'When reasoning is enabled, llama.cpp enforces the token budget server-side and EXL3 enforces it in SiftKit by stopping the stream at the budget and continuing with the budget message closed into the think block.' },
      { label: 'ReasoningBudgetMessage', layout: 'full', helpText: 'The message injected when the reasoning budget is exhausted (llama.cpp `--reasoning-budget-message` flag, or the SiftKit-side continuation on EXL3).' },
