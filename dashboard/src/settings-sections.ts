...
export const SETTINGS_SECTIONS: Record<SettingsSectionId, SettingsSectionDescriptor> = {
  ...
  'model-presets': {
    ...
    fields: [
      ...
      { label: 'SpeculativeNgramMinHits', layout: 'quarter', helpText: 'Minimum repeated matches required before `ngram-map-k` style speculation is used. Use -1 to omit the llama.cpp flag.' },
      { label: 'SpeculativeDraftMax', layout: 'quarter', helpText: 'Upper bound for draft tokens proposed in one speculative step. Use -1 to omit the llama.cpp flag.' },
      { label: 'SpeculativeDraftMin', layout: 'quarter', helpText: 'Minimum accepted draft token count before speculative decoding is used. Use -1 to omit the llama.cpp flag.' },
