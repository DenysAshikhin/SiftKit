import type { ChatSession, DashboardConfig, DashboardPreset, DashboardPresetExecutionFamily, DashboardPresetSurface } from './types';
...
export function getPresetFamily(config: DashboardConfig | null, session: ChatSession | null): DashboardPresetExecutionFamily {
  const preset = getPresetById(config, session?.presetId);
  if (preset) {
    return preset.executionFamily;
  }
