...
$script:PipelineBuffer = New-Object System.Collections.Generic.List[string]
...
function Read-SiftStdin {
    if ($script:PipelineBuffer.Count -gt 0) {
        $pipelineText = $script:PipelineBuffer -join [Environment]::NewLine
...
if ($null -ne $InputObject -and $InputObject.Length -gt 0) {
    [void]$script:PipelineBuffer.Add([string]$InputObject)
}

if ($script:PipelineBuffer.Count -eq 0) {
    foreach ($item in $input) {
        if ($null -ne $item -and [string]$item -ne '') {
            [void]$script:PipelineBuffer.Add([string]$item)
        }
    }
}
...
$knownCommands = @('summary', 'run', 'find-files', 'install', 'test', 'eval', 'codex-policy', 'install-global')
...
switch ($commandName) {
    'summary' {
        ...
        $inputText = if ($parsed.Text) { $parsed.Text } elseif ($parsed.File) { $null } else { Read-SiftStdin }
        $format = if ($parsed.Format) { $parsed.Format } else { 'text' }
        $profile = if ($parsed.Profile) { $parsed.Profile } else { 'general' }

        if (-not $parsed.File -and [string]::IsNullOrWhiteSpace($inputText)) {
