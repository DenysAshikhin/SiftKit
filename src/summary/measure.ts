...
export const UNSUPPORTED_INPUT_MESSAGE =
  'The command/input is either unsupported or failed. Please verify the command that it is supported in the current environment and returns proper input. If it does, raise an explicit error to the user and stop futher processing.';

export function normalizeInputText(text: string | null | undefined): string | null {
...
export function measureText(text: string): { CharacterCount: number; LineCount: number } {
...
export function getQuestionAnalysis(question: string | null | undefined): QuestionAnalysis {
...
export function getErrorSignalMetrics(text: string): ErrorSignalMetrics {
...
export function isPassFailQuestion(question: string | null | undefined): boolean {
  const normalized = question ? question.toLowerCase() : '';
  return (
...
export function getDeterministicExcerpt(
