...
function getStatusArtifactPath(metadata: StatusMetadata): string | null {
...
// jsonl-transcript helpers moved to ../state/jsonl-transcript.ts
...
function loadDashboardRuns(runtimeRoot: string): RunRecord[] {
...
function buildDashboardRunDetail(runtimeRoot: string, runId: string): { run: RunRecord; events: JsonlEvent[] } | null {
...
function getPromptCacheHitRate(promptCacheTokens: unknown, promptEvalTokens: unknown): number | null {
...
function getSnapshotTotalsBeforeDate(database: DatabaseInstance | null, dateKey: string): SnapshotTotals | null {
  if (!database) {
    return null;
  }
  const row = database
...
function buildLiveTodayMetrics(currentMetrics: Metrics, idleSummaryDatabase: DatabaseInstance | null): DailyMetrics {
...
function buildDashboardDailyMetricsFromRuns(runtimeRoot: string): DailyMetrics[] {
  const runs = loadDashboardRuns(runtimeRoot);
...
function buildDashboardDailyMetricsFromIdleSnapshots(database: DatabaseInstance | null): DailyMetrics[] {
  if (!database) {
    return [];
  }
  const rows = database
...
function buildDashboardDailyMetrics(runtimeRoot: string, idleSummaryDatabase: DatabaseInstance | null, currentMetrics: Metrics): DailyMetrics[] {
...
type IdleSummarySnapshotRow = IdleSummarySnapshot & { summaryText: string };

function normalizeIdleSummarySnapshotRow(row: Dict | null): IdleSummarySnapshotRow | null {
  if (!row || typeof row !== 'object') {
    return null;
  }
...
function getChatSessionsRoot(runtimeRoot: string): string {
  return path.join(runtimeRoot, 'chat', 'sessions');
}

function listChatSessionPaths(runtimeRoot: string): string[] {
  return listFiles(getChatSessionsRoot(runtimeRoot))
    .filter((targetPath) => /^session_.+\.json$/iu.test(path.basename(targetPath)));
}

type ChatMessage = Dict;
type ChatSession = Dict & { id: string; messages?: ChatMessage[]; hiddenToolContexts?: Dict[] };

function readChatSessionFromPath(targetPath: string): ChatSession | null {
...
function readChatSessions(runtimeRoot: string): ChatSession[] {
...
function resolveActiveChatModel(config: Dict | null | undefined, session: ChatSession): string {
...
type ChatCompletionRequest = { url: string; model: string; body: Dict };
type BuildChatOptions = { thinkingEnabled?: boolean; stream?: boolean };

function buildChatCompletionRequest(config: Dict, session: ChatSession, userContent: string, options: BuildChatOptions = {}): ChatCompletionRequest {
...
async function generateChatAssistantMessage(config: Dict, session: ChatSession, userContent: string): Promise<{ assistantContent: string; thinkingContent: string; usage: ChatUsage }> {
...
function appendChatMessagesWithUsage(
  runtimeRoot: string,
  session: ChatSession,
  content: string,
...
type StreamProgress = { assistantContent: string; thinkingContent: string };
type StreamResult = { assistantContent: string; thinkingContent: string; usage: ChatUsage };

async function streamChatAssistantMessage(
  config: Dict,
  session: ChatSession,
  userContent: string,
...
function condenseChatSession(runtimeRoot: string, session: ChatSession): ChatSession {
  const now = new Date().toISOString();
  const messages = Array.isArray(session.messages) ? session.messages.slice() : [];
...
function buildPlanRequestPrompt(userPrompt: unknown): string {
...
function buildPlanMarkdownFromRepoSearch(userPrompt: string, repoRoot: string, result: Dict | null | undefined): string {
...
function getScorecardTotal(scorecard: unknown, key: string): number | null {
...
function buildToolContextFromRepoSearchResult(result: Dict | null | undefined): string[] {
  const scorecard = result && typeof result.scorecard === 'object' ? result.scorecard as Dict : {};
  const tasks = Array.isArray(scorecard.tasks) ? scorecard.tasks as Dict[] : [];
...
function buildRepoSearchMarkdown(userPrompt: string, repoRoot: string, result: Dict | null | undefined): string {
  const scorecard = result && typeof result.scorecard === 'object' ? result.scorecard as Dict : {};
  const tasks = Array.isArray(scorecard.tasks) ? scorecard.tasks as Dict[] : [];
  const primaryTask = tasks[0] && typeof tasks[0] === 'object' ? tasks[0] : null;
  const modelOutput = typeof primaryTask?.finalOutput === 'string' && (primaryTask.finalOutput as string).trim()
