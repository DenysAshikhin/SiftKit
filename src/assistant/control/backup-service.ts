...
export class BackupService {
  ...
  async createBackup(): Promise<Buffer> {
    ...
    add('snapshot.sqlite', await this.snapshot());
    for (const [name, bytes] of this.blobFiles()) add(name, bytes);
    add('key.protected', await dpapiProtect(
  ...
  private blobFiles(): Map<string, Buffer> {
    const root = assistantEvidenceDir(this.runtimeRoot);
    ...
    for (const entry of fs.readdirSync(root, { recursive: true, withFileTypes: true })) {
      ...
      files.set(`blobs/${relative}`, fs.readFileSync(absolute));
    }
    return files;
  }
}
