...
export class ChatRuntimeOwner {
  constructor(readonly databasePath: string, readonly ownerId: string, readonly epoch: number) {}
  get ownerEpoch(): string { return `${this.ownerId}:${this.epoch}`; }
  static acquire(databasePath: string, ownerId: string, nowMs = Date.now()): ChatRuntimeOwner {
    ...
    return database.transaction(() => {
      ...
      return new ChatRuntimeOwner(databasePath, ownerId, epoch);
    })();
  }
  renew(nowMs = Date.now()): void {
    ...
    database.transaction(() => {
      ...
      database.prepare('UPDATE chat_runtime_owner SET heartbeat_at_utc=?, lease_expires_at_utc=? WHERE id=1 AND owner_id=? AND epoch=?')
  ...
  assertOwned(nowMs = Date.now()): void {
    const row = ChatRuntimeOwnerSchema.parse(getRuntimeDatabase(this.databasePath).prepare('SELECT * FROM chat_runtime_owner WHERE id=1').get());
  ...
  release(nowMs = Date.now()): void {
    getRuntimeDatabase(this.databasePath).prepare('UPDATE chat_runtime_owner SET lease_expires_at_utc=? WHERE id=1 AND owner_id=? AND epoch=?')
