...
exports.getRepoRuntimeRoot = getRepoRuntimeRoot;
exports.getRuntimeDatabasePath = getRuntimeDatabasePath;
exports.getRuntimeDatabase = getRuntimeDatabase;
exports.closeRuntimeDatabase = closeRuntimeDatabase;
exports.getRuntimeMetadataValue = getRuntimeMetadataValue;
exports.pruneRuntimeHistory = pruneRuntimeHistory;
exports.setRuntimeMetadataValue = setRuntimeMetadataValue;
...
function detectEffectiveSchemaVersion(database, storedVersion) {
    if (storedVersion >= 13) {
...
function getSchemaVersion(database) {
  ...
  `);
    const row = database.prepare('SELECT version FROM runtime_schema WHERE id = 1').get();
...
function ensureRuntimeArtifactsSchema(database) {
    database.exec(`
    CREATE TABLE IF NOT EXISTS runtime_artifacts (
      id TEXT PRIMARY KEY,
      artifact_kind TEXT NOT NULL,
...
function ensureChatMessageTimelineSchema(database) {
...
function ensureManagedLlamaAndBenchmarkMatrixSchema(database) {
    database.exec(`
...
function ensureDashboardBenchmarkSchema(database) {
    database.exec(`
...
function ensureRuntimeMetricsTotalsSchema(database) {
    database.exec(`
    CREATE TABLE IF NOT EXISTS runtime_metrics_totals (
      id INTEGER PRIMARY KEY CHECK (id = 1),
      schema_version INTEGER NOT NULL,
...
function migrateStoredPlannerToolNames(database) {
  ...
  `).get();
    ...
    let parsedOperationModeAllowedTools = undefined;
    let parsedPresets = undefined;
    try {
    ...
    WHERE id = 1
  `).run(JSON.stringify((0, presets_js_1.normalizeOperationModeAllowedTools)(parsedOperationModeAllowedTools)), JSON.stringify((0, presets_js_1.normalizePresets)(parsedPresets)));
}
const V26_DROPPED_APP_CONFIG_COLUMNS = [
 ...
 */
function migrateAppConfigToPresetSourceOfTruth(database) {
    if (!tableExists(database, 'app_config')) {
        return;
    }
    if (tableHasColumn(database, 'app_config', 'server_llama_presets_json')) {
        const row = database.prepare('SELECT server_llama_presets_json AS presetsJson FROM app_config WHERE id = 1').get();
        let presets = [];
        try {
            presets = row?.presetsJson ? JSON.parse(row.presetsJson) : [];
        }
        catch {
            presets = [];
        }
...
function ensureSchema(database) {
    ...
    if (currentVersion <= 0) {
        ...
        setSchemaVersion(database, exports.CURRENT_SCHEMA_VERSION);
        return;
    }
    applyBaseSchema(database);
...
function getRepoRuntimeRoot(startPath = process.cwd()) {
    const repoRoot = (0, paths_js_1.findNearestSiftKitRepoRoot)(startPath);
    const resolvedBaseRoot = repoRoot ? path.resolve(repoRoot) : path.resolve(startPath);
    return path.join(resolvedBaseRoot, '.siftkit');
}
function getRuntimeDatabasePath(startPath = process.cwd()) {
    return path.join(getRepoRuntimeRoot(startPath), 'runtime.sqlite');
}
function configureRuntimeDatabase(database) {
    database.exec(`
    PRAGMA journal_mode = WAL;
    PRAGMA synchronous = NORMAL;
  `);
}
function closeRuntimeDatabaseHandle(database) {
    try {
        database.exec(`
...
function getRuntimeDatabase(databasePath = getRuntimeDatabasePath()) {
    ...
    (0, fs_js_1.ensureDirectory)(path.dirname(resolvedPath));
    let database = new better_sqlite3_1.default(resolvedPath);
    try {
        ...
        database = new better_sqlite3_1.default(resolvedPath);
        configureRuntimeDatabase(database);
        ensureSchema(database);
    }
...
function getRuntimeMetadataValue(key, databasePath = getRuntimeDatabasePath()) {
...
function pruneRuntimeHistory(retentionDays, databasePath = getRuntimeDatabasePath()) {
    ...
    let vacuumed = false;
    try {
        const freelistRow = database.prepare('PRAGMA freelist_count').get();
        const pageRow = database.prepare('PRAGMA page_count').get();
...
function setRuntimeMetadataValue(key, value, databasePath = getRuntimeDatabasePath()) {
    const normalizedKey = String(key || '').trim();
    if (!normalizedKey) {
  ...
  `).run(normalizedKey, String(value || ''), new Date().toISOString());
}
