...
import { httpClient, HttpResponseError, InferenceHttpError } from '../lib/http-client.js';
...
export type InferenceModelProbeResult = {
  statusCode: number;
  rawText: string;
  models: string[];
};
...
export type InferenceChatOptions = {
  config: SiftConfig;
  baseUrl?: string;
  model: string;
  ...
  maxTokens: number;
  responseFormat?: InferenceChatRequest['response_format'];
...
export class InferenceClient {
  constructor(private readonly client: InferenceHttpClient = httpClient) {}
  ...
  async countTokens(
    ...
    const response = await retryProviderRequest(async () => {
      const nextResponse = await this.client.requestJsonFull({
        url: `${baseUrl.replace(/\/$/u, '')}/v1/token/encode`,
        method: 'POST',
  ...
  async probeModelsAtBaseUrl(baseUrl: string, timeoutMs = 5000): Promise<InferenceModelProbeResult> {
  ...
  private async chatAtBaseUrl(baseUrl: string, options: InferenceChatOptions): Promise<NormalizedInferenceChatResponse> {
    const attempt = async (): Promise<NormalizedInferenceChatResponse> => {
  ...
  private async continueAfterThinkingBudget(
    ...
    streamed: NormalizedInferenceChatResponse,
  ): Promise<NormalizedInferenceChatResponse> {
  ...
  private resolveReasoning(options: InferenceChatOptions): 'on' | 'off' | undefined {
    return options.reasoningOverride
      ?? buildPresetRequestDefaults(getActiveModelPreset(options.config)).reasoning;
  }

  private buildChatRequest(options: InferenceChatOptions, responsePrefix?: string): InferenceChatRequest {
    const activePreset = getActiveModelPreset(options.config);
  ...
  private async streamChatAtBaseUrl(
    ...
    try {
      ...
      if (error instanceof HttpResponseError) {
        throw new InferenceHttpError(error.statusCode, error.rawText);
      }
