...
export function buildTaskSystemPrompt(repoRoot: string, options?: {
  ...
  return [
    ...
    '- Turn 1: pick 5 keywords from the task and run `rg -n "k1|k2|k3|k4|k5"` (no path — searches from the repo root; runtime ignore policy filters noise). If empty, reformulate before drilling.',
    ...
    '',
    'JSON examples:',
    '{"action":"repo_rg","command":"rg -n \\"invokePlannerMode\\""}',
    '{"action":"repo_list_files","path":".","glob":"*.ts","recurse":true}',
    '{"action":"repo_read_file","path":"dir\\\\foo.ts","startLine":861,"endLine":1100}',
