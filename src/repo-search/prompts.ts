...
export function buildTaskSystemPrompt(repoRoot: string, options?: {
  ...
  return [
    ...
    'Use direct JSON tool actions with the repo_* tool name in action and tool arguments at the top level.',
    'When several independent read-only searches are genuinely useful, use one tool_batch JSON object with calls.',
    'Tool action examples: {"action":"repo_rg","command":"rg -n \\"pattern\\" src"} or {"action":"repo_read_file","path":"src/index.ts","startLine":1,"endLine":120}',
    'Tool/command rule: command-based action values must match the command prefix (example: repo_rg -> rg, repo_git -> git).',
    ...
    '  Example: {"action":"repo_rg","command":"rg -n \\"keyword1|keyword2|keyword3|keyword4|keyword5\\" src"}',
    ...
    '{"action":"repo_rg","command":"rg -n \\"buildPlannerToolDefinitions\\" src\\\\summary.ts"}',
    '{"action":"repo_rg","command":"rg -n \\"getConfiguredLlamaNumCtx\\" src tests"}',
    '{"action":"repo_list_files","path":"src","glob":"*.ts","recurse":true}',
    '{"action":"repo_read_file","path":"src\\\\summary.ts","startLine":1,"endLine":240}',
    '{"action":"repo_rg","command":"rg -n \\"invokePlannerMode\\" src\\\\summary.ts"}',
    ...
    '- Invalid tool usage example: `{"action":"read_lines","path":"src/app.ts"}`.',
    '- Invalid args example: `{"action":"repo_rg","cmd":"rg -n \\"x\\" src"}`.',
    '- Invalid args example: `{"action":"repo_rg"}`.',
    '- Note: `--type tsx`/`jsx` are auto-corrected (see Command normalization above).',
    '- Invalid command parameter example: `{"action":"repo_rg","command":"rg -n \\"x\\" src; del file.txt"}`.',
    '- Invalid args example: `{"action":"repo_read_file"}`.',
