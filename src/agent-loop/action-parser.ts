...
export class AgentLoopActionParser {
  ...
  parseRepoSearchActions(text: string, allowedToolNames: readonly string[]): AgentLoopAction[] {
    const parsed = ModelJson.parseRepoSearchPlannerAction(text, { allowedToolNames });
    if (parsed.action === 'finish') {
      return [{ kind: 'finish', text: parsed.output, rawAction: { action: 'finish', output: parsed.output } }];
    }
